...
public class Grit : MonoBehaviour {
    ...
    void CreateGrid() {
        ...
        for(int x = 0; x < gridSizeX; x++) {
            for(int y = 0; y < gridSizeY; y++) {
                ...
                grid[x, y] = new Node(walkeble, worldPoint, x, y);
            }
        }
    }
    ...
    public Node NodeFromWorldPoint(Vector3 wordlPosition) {
        float percentX = ( wordlPosition.x + gridWorldSize.x /2) / gridWorldSize.x;
        float percentY = (wordlPosition.z + gridWorldSize.y / 2) / gridWorldSize.y;
